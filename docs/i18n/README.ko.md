...
**Claude Code**의 MCP 서버 설정과 지시 파일(CLAUDE.md)을 **Gemini CLI**, **Codex CLI**, **OpenCode**, **Kiro CLI**, **Cursor**, **Kimi CLI**, **Vibe CLI**(Mistral), **Aider CLI**로 동기화합니다.
...
Claude Code를 주요 AI 코딩 에이전트로 사용하면서 무료 티어나 다른 모델을 활용하기 위해 다른 에이전트(Gemini CLI, Codex CLI, OpenCode, Kiro, Cursor, Kimi CLI, Vibe CLI, Aider CLI)도 함께 사용한다면, 각 도구마다 MCP 설정 형식이 다르고 하나씩 설정하는 것이 얼마나 번거로운지 아실 겁니다.
...
marketplace를 통해 Claude Code 플러그인 설치:
...
# 모든 대화에서 slash commands 사용:
#   /sync               — MCP 설정 동기화 (dry-run 미리보기 포함)
#   /sync-list          — 모든 MCP 서버 목록 보기
#   /sync-diff          — 각 에이전트 간 설정 차이 비교
#   /sync-doctor        — MCP 설정 드리프트 감지
#   /sync-validate      — 스키마 및 대상 호환성 검증
#   /sync-reconcile     — 검증 + 드리프트 감지 + 누락분만 동기화
#   /sync-instructions  — CLAUDE.md를 다른 에이전트에 동기화
#   /report-schema      — 리포트 JSON 스키마 문서 생성/출력
```
...
# 지시 파일 동기화 (CLAUDE.md → GEMINI.md / AGENTS.md / Kiro steering / Cursor rules / Aider conventions)
...
| **Vibe Writer** | JSON → TOML `[[mcp_servers]]`, transport 필드 필수, `${VAR:-default}` → 확장 |
...
| Kimi | `~/.kimi/AGENTS.md` | `./AGENTS.md` (Codex / OpenCode / Vibe와 공유) | 그대로 복사 (독립 라인 `@import` 확장) |
| Vibe | `~/.vibe/AGENTS.md` | `./AGENTS.md` (Codex / OpenCode / Kimi와 공유) | 그대로 복사 (독립 라인 `@import` 확장) |
| Aider | `~/.aider/CONVENTIONS.md` | `.aider/CONVENTIONS.md` | 그대로 복사 + `.aider.conf.yml` `read` 자동 갱신 |
...
- Aider 동기화 시 `.aider.conf.yml`의 `read`도 자동 갱신되어 `CONVENTIONS.md`를 global/project 범위에서 자동 로드합니다.
...
| Vibe CLI (글로벌) | `~/.vibe/config.toml` | TOML |
| Vibe CLI (프로젝트) | `.vibe/config.toml` (`--vibe-home ./.vibe` 사용) | TOML |
...
이 프로젝트는 Claude Code **플러그인**이자 **마켓플레이스**로 사용할 수 있으며, Claude Code 대화에서 직접 slash commands와 컨텍스트 인식 skill을 제공합니다.

### 설치

```bash
# GitHub에서 설치 (원격 — 저장소 clone)
claude plugin marketplace add Leoyang183/sync-agents-settings
claude plugin install sync-agents-settings

# 또는 로컬 경로에서 설치 (symlink — 로컬 변경 즉시 반영)
...
| `/sync-doctor` | Claude와 대상 간 MCP 설정 드리프트 감지 |
| `/sync-validate` | MCP 스키마 및 대상 기능 호환성 검증 |
| `/sync-reconcile` | 검증 + 드리프트 감지 + 누락 서버만 동기화 |
