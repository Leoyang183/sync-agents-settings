...
export function runDoctor(targets: SyncTarget[], options: DoctorOptions = {}): DoctorReport {
  ...
  for (const target of targets) {
    const readResult = readTargetNames(
      target,
      options.codexHome,
      options.kimiHome,
      options.vibeHome
...
function readTargetNames(
  ...
  if (target === "vibe") {
    return readTomlTargetNames(resolveVibeConfigPath(vibeHome), (parsed) => {
      const servers = parsed.mcp_servers as Array<{ name: string }> | undefined;
      return (servers ?? []).map((s) => s.name);
    });
  }

  if (target === "codex") {
    return readTomlTargetNames(resolveCodexConfigPath(codexHome), (parsed) => {
...
function readTomlTargetNames(
  configPath: string,
  extractNames: (parsed: TOML.JsonMap) => string[]
): ReadNamesResult {
  ...
  try {
    ...
    return { status: "ok", names: new Set(extractNames(parsed)) };
  } catch (error) {
    return {
      status: "error",
      names: new Set(),
